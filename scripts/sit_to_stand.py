...
def get_target_torques(theta_1: float, theta_2: float, profiles: pd.DataFrame) -> tuple:
    ...
    return tau_1, tau_2, P_EE, index, closest_point
...
def calibrate_height(motor_1: CubemarsMotor, motor_2: CubemarsMotor, freq: int):
    ...
    try:
        motor_1.send_torque(desired_torque=3, safety=True)
        motor_2.send_torque(desired_torque=1.5, safety=True)
...
def main(motor_1: CubemarsMotor, motor_2: CubemarsMotor, freq: int):
    ...
    try:
        for t in loop:
            ...
            tau_1, tau_2, P_EE, index, closest_point = get_target_torques(
            ...
            if t > 0.1:
                task_logger.writerow([t, index, tau_1, tau_2, motor_1.torque, motor_2.torque, P_EE[0], P_EE[1]])
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == '1':
            with CubemarsMotor(motor_type="AK70-10", logging=False, frequency=freq) as motor_1:
                with CubemarsMotor(motor_type="AK60-6", logging=False, frequency=freq) as motor_2:
                    calibrate_height(motor_1, motor_2, freq=freq)
        elif choice == '2':
            with CubemarsMotor(motor_type="AK70-10", logging=logging, frequency=freq) as motor_1:
                with CubemarsMotor(motor_type="AK60-6", logging=logging, frequency=freq) as motor_2:
                    main(motor_1, motor_2, freq=freq)
        elif choice == '0':
            print("Exiting...")
            break
